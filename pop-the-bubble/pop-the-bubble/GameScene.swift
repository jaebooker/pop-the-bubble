...
class GameScene: SKScene {
    ...
    func generateBubbleIn() {
        let timeInterval = TimeInterval((0.5...1.2).random())
    ...
    func generateBubble() -> SKShapeNode {
        ...
        let bubble = SKShapeNode(
            circleOfRadius: (30.0...100.0).random()
        )
        ...
        let bubbleColor = UIColor(
            hue: (0.0...360.0).random(),
            saturation: (30.0...100.0).random(),
            brightness: (60...100).random(),
            alpha: 1
        )
        ...
        let xPos = (leadingXPos...trailingXPos).random()
    ...
    func moveBubbleToTop(bubble: SKShapeNode) {
        ...
        let translateAction = SKAction.move(
            to: locationInScene,
            duration: TimeInterval((1.0...1.5).random())
        )
